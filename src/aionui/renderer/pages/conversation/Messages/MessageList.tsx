...
const MessageList: React.FC<{ className?: string; emptySlot?: React.ReactNode }> = ({ className, emptySlot }) => {
  ...
  if (processedList.length === 0 && emptySlot) {
    return <div className='relative flex-1 h-full min-h-0 flex items-center justify-center'>{emptySlot}</div>;
  }

  return (
    <div className={classNames('relative flex-1 h-full min-h-0', className)}>
      ...
      <Image.PreviewGroup actionsLayout={['zoomIn', 'zoomOut', 'originalSize', 'rotateLeft', 'rotateRight']}>
        <ImagePreviewContext.Provider value={{ inPreviewGroup: true }}>
          <div
            ...
            className='flex-1 h-full min-h-0 overflow-y-auto pb-10px box-border -mx-20px px-20px'
