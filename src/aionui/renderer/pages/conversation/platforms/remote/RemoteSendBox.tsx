...
const RemoteSendBox: React.FC<{ conversation_id: string }> = ({ conversation_id }) => {
  ...
  useEffect(() => {
    const storageCandidates = [
      {
        storageKey: `remote_initial_message_${conversation_id}`,
        processedKey: `remote_initial_processed_${conversation_id}`,
      },
      {
        storageKey: `acp_initial_message_${conversation_id}`,
        processedKey: `acp_initial_processed_${conversation_id}`,
      },
    ];

    const processInitialMessage = async () => {
      const matchedCandidate = storageCandidates.find(({ storageKey, processedKey }) => {
        return sessionStorage.getItem(storageKey) && !sessionStorage.getItem(processedKey);
      });
      if (!matchedCandidate) return;

      const { storageKey, processedKey } = matchedCandidate;
